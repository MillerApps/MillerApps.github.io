...
1. **Download and Install AdGuard Home:** ### [Automated install (Linux/Unix/MacOS/FreeBSD/OpenBSD)](https://github.com/AdguardTeam/AdGuardHome#automated-install-linuxunixmacosfreebsdopenbsd)
...
3. **Configure Encryption:**
   ...
   - Setup Lets Encrypt certs using Certbot:
     ...
     3. ```sh
        ...
        ```
     4. Follow instructions in Certbot
     5. For the domain section, use your domain you purchased. Add a \*<your-domain> for a wildcard cert as well as the base domain <your-domain> separated by a ','.
     6. The final result will give you two files: - `fullchain.pem` – PEM-encoded SSL certificate. - `privkey.pem` – PEM-encoded private key.
     7. Either paste the contents of these files into the fields or provide the full path.
...
1. **Install Tailscale on pfSense:**
   Follow the Tailscale documentation to install and configure Tailscale on pfSense using the built-in package manager. Located at: **System** -> **Package Manager** -> **Available Packages**
...
3. **Navigate to [Tailscale admin](https://login.tailscale.com/admin/machines)**
   ...
   - Next, **Add nameserver** -> **Custom**: add the route you advertised for pfSense.
   - Optinally back on the Machines page
     -1) Click on the three vertical dots (ellipsis) next to the machine's (pfsense) details to open the options menu. 2) In the dropdown menu, click on "Disable key expiry."
